...
export class UserMethods {
    static fieldsValidForLogin = (user: User) => UserMethods.emailIsValid(user) &&
        UserMethods.passwordIsValid(user, true);

    static fieldsValidForRegister = (user: User) => UserMethods.firstNameIsValid(user) &&
    ...
    static passwordIsValid = (user: User, isLogin = false) => !!user.password &&
        !(!isLogin && !/^(?=.*[A-Z])(?=.*[0-9])(?=.*[a-z]).{7,}$/.test(user.password));
