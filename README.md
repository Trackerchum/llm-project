...
This is a monorepo containing multiple frontend, backend, infrastructure projects, and an Ollama LLM that are developed and run together in a single Docker Compose environment. Node ≥22 and npm ≥10 are required on the host only for local tooling and IDE support; all runtime code executes inside Linux Docker containers. The package manager is npm and uses workspaces, to install a package within a workspace use `npm install -w ./src/<path-to-project> <package-name>` in the root of the repository. The key components and projects are as follows: 
...
- An Ollama LLM instance running model llama3.1:8b.
